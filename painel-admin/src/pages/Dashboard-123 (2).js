...
// Função para obter equipamentos excedentes
function getExcedentEquipments(schools, category, totalExpected, startDate, endDate) {
  const excedentItems = [];
  ...
  schools.forEach(school => {
    (school.tasks || []).forEach(task => {
      ...
      if (inSelectedMonth && isFinished && matchesCategory(task, category)) {
        const equipCount = getEquipmentCount(task) || 1;
        excedentItems.push({
          school: school.school_info?.name || 'Escola desconhecida',
          task: task.orientation || 'Tarefa sem orientação',
...
// Função para criar categorias de KPI baseadas no total real de equipamentos
const createKpiCategories = (totalEquipments) => ({
  MENSAL: {
    ...
    denominator: totalEquipments,
  },
  SEMESTRAL: {
    ids: [TASK_TYPE_IDS.PREVENTIVA_SEMESTRAL],
    keywords: ['semestral', 'preventiva semestral'],
    denominator: Math.round(totalEquipments / 6), // 6 meses a partir de março
  },
  PMOC: {
...
function computeCustomKpis(schools, startDate, endDate, totalEquipments = 0) {
  // Usar o total real de equipamentos ou calcular baseado nas escolas
  const realTotalEquipments = totalEquipments || schools.reduce((total, school) => {
    return total + (school.equipments ? school.equipments.filter(e => 
      e.active === 1 || e.ativo === 1 || e.active === true || e.ativo === true || 
      e.active === 'Sim' || e.ativo === 'Sim'
    ).length : 0);
  }, 0);

  // Criar categorias com o total real de equipamentos
  const KPI_CATEGORIES = createKpiCategories(realTotalEquipments);
...
const renderCustomKpiCards = (k, openExDialog, mensalItems=[], semestralItems=[], pmocItems=[]) => {
  ...
  console.log('k.mensalDone:', k.mensalDone);
  console.log('k.mensalTotal:', k.mensalTotal);
  console.log('k.semestralDone:', k.semestralDone);
  console.log('k.semestralTotal:', k.semestralTotal);
  console.log('k.pmocDone:', k.pmocDone);
  console.log('k.pmocTotal:', k.pmocTotal);
  console.log('========================');
  
  return (
    <Box sx={{ mb: 2 }}>
      <Grid container spacing={2}>
        <Grid item xs={12} sm={6} md={3}><KpiCard title="Prev. Mensal" value={`${k.mensalDone}/${k.mensalTotal} ${k.mensalPct}%`} icon={<EventRepeatIcon color="success" />} extra={k.mensalEx > 0 && (
      ...
      <Grid item xs={12} sm={6} md={3}><KpiCard title="Prev. Semestral" value={`${k.semestralDone}/${k.semestralTotal} ${k.semestralPct}%`} icon={<CalendarViewWeekIcon color="info" />} extra={k.semestralEx > 0 && (
      ...
      <Grid item xs={12} sm={6} md={3}><KpiCard title="PMOC" value={`${k.pmocDone}/${k.pmocTotal} ${k.pmocPct}%`} icon={<ArticleIcon color="warning" />} extra={k.pmocEx > 0 && (
    ...
    <Grid container spacing={2}>
      <Grid item xs={12} sm={6} md={3}><KpiCard title="Total de Tarefas" value={indicators.total_tasks} icon={<AssignmentIcon color="primary" />} /></Grid>
        ...
        <KpiCard 
          ...
          value={(() => {
            if (dashboardData && dashboardData.schools) {
              // Soma todos os equipamentos ativos de todas as escolas exibidas
              return dashboardData.schools.reduce((acc, school) => {
                if (!school.equipments) return acc;
                return acc + school.equipments.filter(e => e.active === 1 || e.ativo === 1 || e.active === true || e.ativo === true || e.active === 'Sim' || e.ativo === 'Sim').length;
              }, 0);
            }
            return indicators.total_equipments || 0;
          })()}
          icon={<BuildIcon color="action" />}
        />
      </Grid>
      <Grid item xs={12} sm={6} md={3}><KpiCard title="Colaboradores" value={indicators.total_collaborators ?? (dashboardData.collaborators ? dashboardData.collaborators.length : 0)} icon={<PeopleIcon color="action" />} /></Grid>
      <Grid item xs={12} sm={6} md={3}>
        <KpiCard 
          ...
          <Box sx={{ 
            ...
            {(() => {
                ...
                const collaboratorsToShow = dashboardData.collaborators.filter(c => {
                  ...
                  const pmocIds = [TASK_TYPE_IDS.PMOC];
                  const allPmoc = collabTasks.every(t => {
                    const orientation = (t.orientation || '').toLowerCase();
                    return pmocIds.includes(t.taskType) || orientation.includes('pmoc');
                  });
                  return !allPmoc;
                });
  ...
  const indicators = processedData ? processedData.indicators : {};
    ...
    return (
      <Box>
        ...
        {(() => {
          // Calcular o total real de equipamentos do contrato atual
          const totalEquipments = indicators.total_equipments || 0;
          const kpiData = computeCustomKpis(filteredSchools, dateRange.start, dateRange.end, totalEquipments);
          
          // Criar categorias com o total real para os excedentes
          const realKpiCategories = createKpiCategories(totalEquipments);
          const mensalItems = getExcedentEquipments(filteredSchools, realKpiCategories.MENSAL, kpiData.mensalTotal, dateRange.start, dateRange.end);
          const semestralItems = getExcedentEquipments(filteredSchools, realKpiCategories.SEMESTRAL, kpiData.semestralTotal, dateRange.start, dateRange.end);
          const pmocItems = getExcedentEquipments(filteredSchools, realKpiCategories.PMOC, kpiData.pmocTotal, dateRange.start, dateRange.end);
