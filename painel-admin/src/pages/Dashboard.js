...
// Função para criar categorias de KPI com base no total de equipamentos do contrato
...
function computeCustomKpis(schools, startDate, endDate, totalEquipments) {
  // Criar categorias dinâmicas com base no total de equipamentos do contrato
  const KPI_CATEGORIES = createKpiCategories(totalEquipments);
  ...
  const mensalPct = mensalTotal > 0 ? Math.min(100, Math.round((mensalEquip / mensalTotal) * 100)) : 0;
  const semestralPct = semestralTotal > 0 ? Math.min(100, Math.round((semestralEquip / semestralTotal) * 100)) : 0;
  const pmocPct = pmocTotal > 0 ? Math.min(100, Math.round((pmocEquip / pmocTotal) * 100)) : 0;
...
const Dashboard = () => {
    ...
    const fetchAllContractData = async () => {
      ...
      try {
        ...
        const responses = await Promise.all(contractPromises);
          ...
          const calculatedKpis = computeCustomKpis(schools, dateRange.start, dateRange.end, total_equipamentos_ativos);
  ...
  const processedData = useMemo(() => {
    ...
    const indicators = {
      ...
      total_equipments: dashboardData.indicators?.total_equipments || 0, // Usa o valor real do backend
    };
...
// Renderiza o cabeçalho com seletor de contrato e datas
  ...
  const renderFilters = (indicators) => (
    dashboardData && (
        ...
        <Paper variant="outlined" sx={{ 
          ...
          <Box sx={{ 
            ...
            {(() => {
                ...
                const kpiCategories = createKpiCategories(totalEquipments);
                const collaboratorsToShow = dashboardData.collaborators.filter(c => {
                  ...
                  const allPmoc = collabTasks.every(t => matchesCategory(t, kpiCategories.PMOC));
                  return !allPmoc;
                });
  ...
  const filteredSchools = useMemo(() => {
    ...
    return dashboardData.schools.filter(school => {
      // Filtro de colaborador
      const colaboradorOk = !selectedCollaborator ||
        school.tasks.some(task => task.userId === selectedCollaborator);
      // Filtro de tipo de tarefa
      const tipoOk = taskTypeFilter === 'all' ||
        school.tasks.some(task => String(task.taskTypeId) === String(taskTypeFilter));
      return colaboradorOk && tipoOk;
    });
  }, [dashboardData, selectedCollaborator, taskTypeFilter]);
    ...
    return (
      <Box>
        ...
        {(() => {
          ...
          const mensalItems = getExcedentEquipments(filteredSchools, kpiCategories.MENSAL, kpiData.mensalTotal, dateRange.start, dateRange.end);
          const semestralItems = getExcedentEquipments(filteredSchools, kpiCategories.SEMESTRAL, kpiData.semestralTotal, dateRange.start, dateRange.end);
          const pmocItems = getExcedentEquipments(filteredSchools, kpiCategories.PMOC, kpiData.pmocTotal, dateRange.start, dateRange.end);
          return renderCustomKpiCards(kpiData, openExDialog, mensalItems, semestralItems, pmocItems);
        })()}

  {renderFilters(indicators)}
