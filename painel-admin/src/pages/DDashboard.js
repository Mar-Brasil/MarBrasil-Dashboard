...
// Renderiza o cabeçalho com seletor de contrato e datas
  ...
  if (selectedContract === 'all') {
    return (
      <Box>
        <Typography variant="h6" align="center" gutterBottom>Progresso de Contratos</Typography>
        {allContractsProgress.loading ? (
          <LinearProgress />
        ) : (
          <List>
            {allContractsProgress.data.map(item => (
              <ListItem divider key={item.id} sx={{ flexDirection: 'column', alignItems: 'flex-start', py: 2 }}>
                <Typography variant="subtitle2" sx={{ mb: 0.5 }}>{item.name}</Typography>
            ...
            {allContractsProgress.data.length === 0 && (
