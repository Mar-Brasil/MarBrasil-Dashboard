...
async def startup_event():
    ...
    # Tabela de associação N-N entre usuários e contratos
    conn.execute('''
...
def listar_usuarios():
    ...
    users_raw = conn.execute("SELECT * FROM usuarios_painel").fetchall()
    users = []
    for u in users_raw:
        ...
        contratos_cur = conn.execute("SELECT contrato_id FROM usuario_contratos WHERE usuario_id = ?", (u['id'],))
        u["contratos"] = [row['contrato_id'] for row in contratos_cur.fetchall()]
        users.append(u)
...
def criar_usuario(usuario: UsuarioIn):
    ...
    try:
        ...
        # Lidar com associações de contrato
        if usuario.contratos:
            for contrato_id in usuario.contratos:
                conn.execute("INSERT INTO usuario_contratos (usuario_id, contrato_id) VALUES (?, ?)", (user_id, contrato_id))
            conn.commit()
        ...
        # Buscar contratos associados para a resposta
        contratos_cur = conn.execute("SELECT contrato_id FROM usuario_contratos WHERE usuario_id = ?", (user_id,))
        user["contratos"] = [row['contrato_id'] for row in contratos_cur.fetchall()]
...
def editar_usuario(id: int, usuario: UsuarioIn):
    ...
    # Busca o usuário existente para verificar as permissões atuais
    user_atual = conn.execute("SELECT * FROM usuarios_painel WHERE id = ?", (id,)).fetchone()
    if not user_atual:
        raise HTTPException(status_code=404, detail="Usuário não encontrado.")

    # Lógica de permissão simplificada:
    # Por enquanto, vamos assumir que o painel é usado por um admin.
    # A lógica principal é que um usuário não pode escalar os próprios privilégios.
    # Então, ao editar, mantemos as permissões e contratos existentes.
    permissoes_json = user_atual['permissoes'] # Mantém as permissões antigas
    
    # A senha pode ser atualizada
    senha_hash = bcrypt.hashpw(usuario.senha.encode(), bcrypt.gensalt()).decode() if usuario.senha else user_atual['senha_hash']
    ...
    try:
        conn.execute(
            ...
            (
                ...
                usuario.username,
                senha_hash,
                permissoes_json, # Usa as permissões já existentes
                id
            )
        )

        # Atualizar associações de contrato (somente se enviado pelo admin)
        # Como não temos login, vamos permitir a alteração por enquanto, 
        # mas a lógica de permissões acima já é um passo.
        conn.execute("DELETE FROM usuario_contratos WHERE usuario_id = ?", (id,))
        if usuario.contratos:
            for contrato_id in usuario.contratos:
                conn.execute("INSERT INTO usuario_contratos (usuario_id, contrato_id) VALUES (?, ?)", (id, contrato_id))
        ...
        # Buscar contratos associados para a resposta
        contratos_cur = conn.execute("SELECT contrato_id FROM usuario_contratos WHERE usuario_id = ?", (id,))
        user["contratos"] = [row['contrato_id'] for row in contratos_cur.fetchall()]
...
def get_dashboard_batch(request: DashboardBatchRequest):
    ...
    try:
        ...
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Erro ao processar lote de dashboards: {e}")
...
def login(login_data: LoginRequest):
    conn = get_db_connection()
    try:
        user_db = conn.execute("SELECT * FROM usuarios_painel WHERE LOWER(username) = ?", (login_data.username.lower(),)).fetchone()
        if not user_db:
            raise HTTPException(status_code=401, detail="Usuário ou senha inválidos")

        if not bcrypt.checkpw(login_data.senha.encode(), user_db['senha_hash'].encode()):
            raise HTTPException(status_code=401, detail="Usuário ou senha inválidos")

        # Se autenticado, prepare a resposta
        user_db["permissoes"] = json.loads(user_db["permissoes"])
        del user_db["senha_hash"]

        # Buscar contratos associados para a resposta
        contratos_cur = conn.execute("SELECT contrato_id FROM usuario_contratos WHERE usuario_id = ?", (user_db['id'],))
        user_db["contratos"] = [row['contrato_id'] for row in contratos_cur.fetchall()]
